...
export async function generateAcquittalPack({ event, club, checkIns, photos, receipts, afpOverrides, treasurerName, generatedByEmail }) {
  ...
  // 3. Attendance pages, re-generated inline using the same helper
  ...
  // 5. Receipts, one per page with a header strip
  for (const r of receipts) {
    const page = pdf.addPage([A4.width, A4.height]);
    ...
    if (imgBytes) {
      ...
      if (img) {
        ...
        page.drawText('(receipt is a PDF, view the original file)', { x: MARGIN, y: A4.height / 2, size: 9, font: regular, color: MUTED });
      }
    }
  }
