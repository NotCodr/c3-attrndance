// CRTWarp, from React Bits (reactbits.dev): an animated CRT plasma with
// scanlines, bloom and a pointer that bends the signal.
//
// The shader is unchanged from upstream. Changes here:
//   - elapsed time comes from performance.now(), because THREE.Clock logs a
//     deprecation warning from three r183 on;
//   - the WebGL context is released on unmount (forceContextLoss), so buttons
//     that mount and unmount through a flow never pile up contexts;
//   - if WebGL is unavailable the component renders nothing and the container's
...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    try {
      ...
      return undefined; // No WebGL: the container's background shows instead.
    }
  ...
  useEffect(() => {
    ...
    backgroundColor, bloom, bloomRadius, brightness, color, curvature, dpr, mouseReact, mouseStrength,
    noise, pixelation, rgbShift, scanlineFrequency, scanlineStrength, speed, fps, vignette,
